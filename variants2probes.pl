...
print SUMMARY "VariantName\tType\tChange\tProtein\tPosition\tRef\tAlt\tSequence\n";
print MINPROBES "VariantName\tType\tChange\tProtein\tPosition\tRef\tAlt\tSequence\n";
   ...
   while(<IN>)
      ...
      if($subtype eq "SNP" && /variant-base:\s+([CATG])/)
         ...
         if($local_use{$flank} ==1)
            {
            print SUMMARY "$name\t$type\t$label\t$gene\t$pos\t$refbase\t$variantbase\t$flank\n";
            }
         ...
         if($used_probe{$flank} ==1)
            {
            print MINPROBES "$name\t$type\t$label\t$gene\t$pos\t$refbase\t$variantbase\t$flank\n";
            }
         }
     }
close IN; 
